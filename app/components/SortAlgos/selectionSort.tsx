...
const SelectionSort = ({ initValues, isAsc, }: { initValues: number[], isAsc: boolean, }) => {
    const { setSortCompleted, audioClickRef, audioTransRef, audioMarkRef } = useContext(MainContext)
    const { isCompleted, nextStep, values, makeSwap } = useSelectionSort(initValues, isAsc)
    ...
    useEffect(() => {
        if (values.findIndex(v => v.isMarkedForSwap) > -1) {
            ...
            audioClickRef.current.play().catch(res => {
                // console.log(res)
            })
